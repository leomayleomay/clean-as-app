...
forgotPasswordCodec :: JsonCodec ForgotPasswordFields
forgotPasswordCodec =
  CAR.object "ForgotPasswordFields"
    { email: Email.codec
    }
...
resetPasswordCodec :: JsonCodec ResetPasswordFields
resetPasswordCodec =
  CAR.object "ResetPasswordFields"
    { password: CA.string
    , password_confirmation: CA.string
    }
...
register baseUrl fields =
  ...
  in
    requestUser baseUrl { endpoint: Register, method }

forgotPassword :: forall m. MonadAff m => BaseURL -> ForgotPasswordFields -> m (Either String Unit)
forgotPassword baseUrl fields =
  let
    method = Post $ Just $ Codec.encode forgotPasswordCodec fields
  in
    do
      res <- liftAff $ request $ defaultRequest baseUrl Nothing { endpoint: ForgotPassword, method }
      case res of
        Left e -> pure $ Left $ printError e
        Right _ -> pure $ Right unit

resetPassword :: forall m. MonadAff m => BaseURL -> String -> ResetPasswordFields -> m (Either String Unit)
resetPassword baseUrl token fields =
  let
    method = Post $ Just $ Codec.encode resetPasswordCodec fields
  in
    do
      res <- liftAff $ request $ defaultRequest baseUrl Nothing { endpoint: ResetPassword { reset_password_token: token }, method }
      case res of
        Left e -> pure $ Left $ printError e
        Right _ -> pure $ Right unit
