...
import CleanAs.Api.Request (BaseURL, RequestOptions, Token, defaultRequest, readToken, writeToken)
...
decodeWithUser
  :: forall m a
   . MonadEffect m
  => MonadStore Action Store m
  => (Maybe Email -> JsonCodec a)
  -> Maybe Json
  -> m (Maybe a)
decodeWithUser codec json = do
  { currentUser } <- getStore
  decode (codec (_.email <$> currentUser)) json
