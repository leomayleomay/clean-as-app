...
import CleanAs.Api.Endpoint (Endpoint(..))
import CleanAs.Api.Request (BaseURL(..), RequestMethod(..), defaultRequest, readToken)
import CleanAs.AppM (runAppM)
import CleanAs.Component.Router as Router
import CleanAs.Data.Profile (Profile)
import CleanAs.Data.Profile as Profile
import CleanAs.Data.Route (routeCodec)
import CleanAs.Store (Store)
...
main = HA.runHalogenAff do
  ...
  currentUser :: Maybe Profile <- (liftEffect readToken) >>= case _ of
    ...
    Just token -> do
      let requestOptions = { endpoint: CurrentUser, method: Get }
  ...
  let
    initialStore :: Store
    initialStore = { baseUrl, currentUser }
