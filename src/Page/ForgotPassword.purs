...
-- | their session.
module CleanAs.Page.ForgotPassword where

import Prelude

import CleanAs.Capability.Navigate (class Navigate, navigate)
import CleanAs.Capability.Resource.User (class ManageUser, forgotPassword)
...
component = F.formless { liftAction: Eval } mempty $ H.mkComponent
  ...
  handleQuery = do
    let
      onSubmit = forgotPassword >=> const (navigate Home)
      validation =
